...
router = APIRouter(
    prefix="/api/v1/travel-requirements",
    tags=["Travel Requirements (v1)"],
)
...
@router.post(
    ...
    summary=(
        "Evaluate travel documentation requirements "
        "for a passenger journey"
    ),
    description=(
        "Accepts a complete passenger profile and journey "
        "itinerary, evaluates all applicable regulatory "
        "rules (visa, passport, transit, health, "
        "immigration, customs, entry restrictions), and "
        "returns structured requirements with provenance."
    ),
)
